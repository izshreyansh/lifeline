...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('global.create') }} {{ trans('cruds.nonProductive.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route("admin.non-productives.store") }}" enctype="multipart/form-data">
                @csrf
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('reason') ? 'is-invalid' : '' }}" name="reason" id="reason" required>
                        ...
                        @foreach(App\NonProductive::REASON_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('reason', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
